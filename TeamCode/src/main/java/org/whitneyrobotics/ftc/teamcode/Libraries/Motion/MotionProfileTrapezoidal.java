...
public class MotionProfileTrapezoidal {
    ...
    private void calculateTimes(){
        ...
        if(this.goalPoint<=Math.pow(maxVelocity.get(),2)/maxAccel.get()){ //Check if goal is less than the max triangular integral without a cruise phase
            t2=t1;
            tf = 2*t1;
        } else {
            t2 = (this.goalPoint-(maxVelocity.get()*t1))/maxVelocity.get()+t1; //Subtract the regions of the acceleration/deceleration triangles from the integral of the trapezoid and divide by rate
            tf = t2+t1;
        }
    }
    ...
    public double positionAt(double t_elapsed){
        ...
        if(t_elapsed <= t1){
            return 0.5*a_max*Math.pow(t_elapsed,2);
        } else if(t_elapsed <= t2){
            return positionAt(t1) + v_max*(t_elapsed-t1);
        } else if(t_elapsed <= tf){
            return positionAt(t2) + v_max*(t_elapsed-t2)+0.5*-a_max*Math.pow(t_elapsed-t2,2);
        } else {
            return goalPoint;
        }
    }
    ...
    public double velocityAt(double t_elapsed){
        double a_max = maxAccel.get(), v_max = maxVelocity.get();
        if(t_elapsed <= t1) return a_max*t_elapsed;
        else if(t_elapsed <= t2) return v_max;
        else if(t_elapsed <= tf) return v_max-a_max*(t_elapsed-t2);
        return 0;
    }

    public double accelerationAt(double t_elapsed){
        double a_max = maxAccel.get();
        if (t_elapsed <= t1) return a_max;
        else if (t_elapsed <= t2) return 0;
        else if (t_elapsed <= tf) return -a_max;
        return 0;
