    ...
    protected void loopInternal() {
        ...
        if (!drivetrain.isBusy()) {
            drivetrain.setWeightedDrivePower(
                    ...
                    new Pose2d(
                            scaling.apply(gamepad1.LEFT_STICK_Y.value()) * (1 - brakePower),
                            scaling.apply(alliance.allianceCoefficient * gamepad1.LEFT_STICK_X.value()) * (1 - brakePower), //When on the left side of the field, our strafe controls are inverted
                            scaling.apply(gamepad1.RIGHT_STICK_X.value()) * (1 - brakePower)
                    )
            );
        }
