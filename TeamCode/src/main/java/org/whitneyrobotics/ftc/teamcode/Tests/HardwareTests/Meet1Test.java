...
public class Meet1Test extends OpModeEx {
    ...
    public void testMotor(Button b, DcMotor m, String motor){
        if(b.value()){
            ...
            telemetryPro.addLine("Motor " + motor + " running.");
        } else {
            m.setPower(0);
        }
    }
    ...
    protected void loopInternal() {
        ...
        telemetryPro.addData("servo position",servoPosition);
        telemetryPro.addData("sL reading",sL.getCurrentPosition());
        telemetryPro.addData("sR reading", sR.getCurrentPosition());

        telemetryPro.addData("first angle",imu.getAngularOrientation().firstAngle);
        telemetryPro.addData("second angle",imu.getAngularOrientation().secondAngle);
        telemetryPro.addData("third angle",imu.getAngularOrientation().thirdAngle);
        telemetryPro.addData("slidesVelocity",sL.getVelocity(AngleUnit.RADIANS));
